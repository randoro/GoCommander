...
public class CommanderSpawner : MonoBehaviour
{
    ...
    IEnumerator GetTreasures()
    {
        ...
        for (int i = 0; i < nav.Length - 1; i++)
        {
            ...
            if (OutsideLngLatRadius(GoogleMap.centerLocation.longitude, GoogleMap.centerLocation.latitude, lng, lat))
            {
    ...
    bool OutsideLngLatRadius(double playerLng, double playerLat, double treasureLng, double treasureLat)
    {
